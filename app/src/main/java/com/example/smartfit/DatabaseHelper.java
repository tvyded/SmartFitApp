...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public Cursor getWorkouts(String user) {
        ...
        return db.rawQuery("SELECT * FROM workouts WHERE user=? AND status='completed' ORDER BY start_date DESC",
                new String[]{user});
    }
    ...
    public Cursor getSets(long workoutId) {
        SQLiteDatabase db = getReadableDatabase();
        return db.rawQuery("SELECT * FROM sets WHERE workout_id=? ORDER BY id", new String[]{String.valueOf(workoutId)});
    }

    public int getExerciseCount(long workoutId) {
        SQLiteDatabase db = getReadableDatabase();
        Cursor c = db.rawQuery("SELECT COUNT(DISTINCT exercise) FROM sets WHERE workout_id=?", new String[]{String.valueOf(workoutId)});
        c.moveToFirst();
    ...
    public int getTotalReps(long workoutId) {
        SQLiteDatabase db = getReadableDatabase();
        Cursor c = db.rawQuery("SELECT SUM(reps) FROM sets WHERE workout_id=?", new String[]{String.valueOf(workoutId)});
        c.moveToFirst();
